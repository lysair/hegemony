...
local jianan = fk.CreateTriggerSkill{
  ...
  events = {fk.EventPhaseStart},
  can_trigger = function (self, event, target, player, data)
    return H.compareKingdomWith(player, target) and not target:isNude() and target.phase == Player.Start and player:hasSkill(self.name)
  end,
  on_cost = function (self, event, target, player, data)
...
Fk:loadTranslationTable{
  ...
  "②当【六龙骖驾】移动至其他角色的装备区内后，你可以交换你与其装备区内的防御坐骑牌。",
  ["jianan"] = "建安",
  ...
  [":jianan__ld__jieyue"] = "准备阶段开始时，你可将一张手牌交给不是魏势力或没有势力的一名角色，对其发起军令。若其：执行，你摸一张牌；不执行，摸牌阶段，你令额定摸牌数+3。",
