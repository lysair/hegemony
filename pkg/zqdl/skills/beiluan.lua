...
Fk:loadTranslationTable{
  ["zq__beiluan"] = "备乱",
  [":zq__beiluan"] = "当你受到伤害后，你可以令伤害来源所有非装备手牌视为【杀】直到当前回合结束。",

  ["#zq__beiluan-invoke"] = "备乱：是否令 %dest 本回合非装备手牌视为【杀】？",
  ["@@zq__beiluan-turn"] = "备乱",
}
...
beiluan:addEffect(fk.Damaged, {
  ...
  on_cost = function (self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#zq__beiluan-invoke::"..data.from.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    player.room:setPlayerMark(data.from, "@@zq__beiluan-turn", 1)
  end
})
...
beiluan:addEffect("filter", {
  ...
  card_filter = function(self, card, player)
    return player:getMark("@@zq__beiluan-turn") > 0 and
