...
chengshang:addEffect(fk.CardUseFinished, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(chengshang.name) and player.phase == Player.Play and data.tos and
      table.find(data.tos, function(p) return not H.compareKingdomWith(p, player) end)
      and not data.damageDealt and data.card.suit ~= Card.NoSuit and player:usedSkillTimes(chengshang.name, Player.HistoryPhase) == 0
  end,
  ...
  on_use = function(self, event, target, player, data)
    ...
    if #cards > 0 then
      room:moveCardTo(cards, Card.PlayerHand, player, fk.ReasonJustMove, chengshang.name, nil, false, player.id)
    else
