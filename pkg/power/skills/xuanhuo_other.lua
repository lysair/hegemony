...
xuanhuo_other:addEffect("active",{
  ...
  can_use = function(self, player)
    return player:usedSkillTimes(xuanhuo_other.name, Player.HistoryPhase) == 0 and table.find(Fk:currentRoom().alive_players, function(p)
  ...
  on_use = function(self, room, effect)
    ...
    local targets = table.filter(room:getOtherPlayers(player, false), function(p) return p:hasShownSkill("ld__xuanhuo") and H.compareKingdomWith(p, player) end)
    ...
    local choice = room:askToChoice(player,{choices =  choices, skill_name = xuanhuo_other.name, prompt = "#ld__xuanhuo-choice", detailed = true, all_choices =  all_choices})
