...
local aocai = fk.CreateViewAsSkill{
  ...
  before_use = function(self, player, use)
    ...
    cards = room:askForCard(player, 1, 1, false, self.name, true, tostring(Exppattern{ id = ids }), "#ld__aocai-choose", cards)
    if #cards > 0 then
      ...
      if not player.dead then
        ...
        if events then
          for i = x + 1, #events, 1 do
            if not room:getPlayerById(events[i].data[1].who).dead then
              room:loseHp(player, 1, self.name)
              break
            end
          end
...
local zhidao_trigger = fk.CreateTriggerSkill{
  ...
  events = {fk.Damage},
  can_trigger = function (self, event, target, player, data)
    if not (player:hasSkill(self) and player == target and player:usedSkillTimes(zhidao.name, Player.HistoryTurn) > 0
      and data.to:getMark("ld__zhidao-turn") > 0 and data.to ~= player and #data.to:getCardIds("hej") > 0) then return false end
    local room = player.room
...
Fk:loadTranslationTable{
  ...
  ["ld__gongqing"] = "公清",
  [":ld__gongqing"] = "锁定技，当你受到伤害时，若伤害来源的攻击范围：小于3，则此伤害改为1；大于3，则此伤害+1。",
...
local huaiyi = fk.CreateActiveSkill{
  ...
  on_use = function(self, room, effect)
    ...
    local targets = room:askForChoosePlayers(player, table.map(table.filter(room:getOtherPlayers(player, false), function(p)
