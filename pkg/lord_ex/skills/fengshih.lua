...
fengshih:addEffect(fk.TargetConfirmed,{
    mute = true,
    can_trigger = function(self, event, target, player, data)
      return target == player and player:hasShownSkill(fengshih.name) and not player:isNude() and #data:getAllTargets() == 1
        and player:getHandcardNum() < data.from:getHandcardNum()
    end,
    on_cost = function (self, event, target, player, data)
      return player.room:askToSkillInvoke(data.from,{skill_name =  fengshih.name, prompt =  "#ld__fengshih_back-ask:" .. player.id})
    end,
    on_use = function(self, event, target, player, data)
      local room = player.room
