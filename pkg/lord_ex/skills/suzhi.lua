...
suzhi:addEffect("targetmod",{
    bypass_distances = function(self, player, skill, card)
        return card and player:hasSkill(suzhi.name) and player.phase ~= Player.NotActive and player:getMark("@ld__suzhi-turn") < 3 and
        card.type == Card.TypeTrick and (not card:isVirtual() or #card.subcards == 0) and player:hasShownSkill(suzhi.name)
    end,
})
