...
local heavenly_army_skill_trig = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    if not (target == player and player:hasSkill(self.name) and player:hasSkill("hongfa")) then return end
...
Fk:loadTranslationTable{
  ...
  [":hongfa"] = "<b><font color='goldenrod'>君主技</font></b>，你拥有“黄巾天兵符”。<br>#<b>黄巾天兵符</b>：<br>" ..
          "①准备阶段开始时，若没有“天兵”，你将牌堆顶的X张牌置于武将牌上（称为“天兵”）（X为群势力角色数）。<br>" ..
          "②你执行的效果中的“群势力角色数”+X（X为“天兵”数）。<br>" ..
